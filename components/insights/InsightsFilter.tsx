...
export default function InsightsFilter({ posts, categories }: InsightsFilterProps) {
  ...
  return (
    <div>
      ...
      <div className="eg-tile flex flex-col gap-5 sm:flex-row sm:items-center justify-between p-5 sm:p-6 mb-0.5">
        {/* Category Pills */}
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-all duration-200 touch-manipulation ${
              !activeCategory
                ...
                : 'bg-field text-navy-600 hover:text-navy-900 hover:bg-gray-200'
            }`}
          >
            All
          </button>
        ...
        <div className="relative w-full sm:w-72">
          <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-navy-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 text-sm bg-field text-navy-900 placeholder:text-navy-600 focus:outline-none focus:ring-2 focus:ring-signal transition-colors"
          />
        </div>
      </div>
      ...
      {(activeCategory || search) && (
        <div className="eg-tile flex items-center gap-3 px-5 py-4 mb-0.5">
      ...
      {/* Results Grid */}
      {filtered.length === 0 ? (
        <div className="eg-tile text-center py-20">
